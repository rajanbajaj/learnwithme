...
var MediaGroup = mongoose.model('MediaGroup');
...
module.exports.createMediaGroup = function (req, res, next) {
    if (req.body && req.body.name && req.body.path) {
        ...
        let dir = mediaUtils.getFinalDirectoryPath(req.body.path + req.body.name);
...
module.exports.updateMediaGroup = function (req, res, next) {
    if (req.params && req.params.mediaGroupId ) {
        Media.findById(req.params.mediaId).exec(function (err, data) {
            ...
            let name = req.body.name ? req.body.name : data.name;
            let path = req.body.path ? req.body.path : data.path;
            let pathFlag = (path === data.path);
            let nameFlag = (name === data.name);

            if (!pathFlag || !nameFlag) {
                let dir = mediaUtils.getFinalDirectoryPath(path + name);
                if (!fs.existsSync(dir)) {
                    fs.mkdir(dir, { recursive: true }, (err) => {
                        if (err) {
                            logger.error(JSON.stringify(err));
                        ...
                        let moveFrom = mediaUtils.getFinalDirectoryPath(data.path + data.name);
                        let moveTo = mediaUtils.getFinalDirectoryPath(path + name);
...
module.exports.deleteMediaGroup = function (req, res, next) {
    if (req.params && req.params.mediaGroupId ) {
        MediaGroup.findById(req.params.mediaGroupId).exec(function (err, data) {
            if (data) {
                let dir = mediaUtils.getFinalDirectoryPath(data.path + data.name);
...
module.exports.updateMedia = function (req, res, next) {
    if (req.params && req.params.mediaId ) {
        ...
        Media.findById(req.params.mediaId).exec(function (err, data) {
            ...
            let filename = req.body.filename ? req.body.filename : data.filename;
            let mediaGroup = req.body.mediaGroupId ? req.body.mediaGroupId : data.mediaGroup;
            let filenameFlag = (filename === data.filename);
            let mediaGroupFlag = (mediaGroup === data.mediaGroup);
                ...
                MediaGroup.findById(data.mediaGroup).exec(function (e, d) {
                    ...
                    if (!mediaGroupFlag) {
                        MediaGroup.findById(req.body.mediaGroupId).exec(function (e, d) {
                            ...
                            let fromPath = path.join(moveFromDir, data.filename);
                            let toPath = path.join(moveToDir, filename);
